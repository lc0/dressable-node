...
//*/
	io.sockets.on('connection', function(client) {
		...
		client.join('dressable');//client.handshake.sid);

		client.send('welcome to dressable: '+user);
		...
		client.on('message', function(msg) {
			...
			io.sockets.in('dressable').send('socket.io relay message "'+msg+'" from: '+ user +' @ '+new Date().toString().match(/[0-9]+:[0-9]+:[0-9]+/));
		});
